...
def test_create_topic_success(client: TestClient, auth_headers, sample_topic_data):
    ...
    response = client.post("/api/topics", json=sample_topic_data, headers=auth_headers)
...
def test_create_topic_without_auth(client: TestClient, sample_topic_data):
    ...
    response = client.post("/api/topics", json=sample_topic_data)
...
def test_create_topic_invalid_answers(client: TestClient, auth_headers):
    ...
    response = client.post("/api/topics", json=invalid_data, headers=auth_headers)
...
def test_create_private_topic_success(client: TestClient, auth_headers, private_topic_data):
    ...
    response = client.post("/api/topics", json=private_topic_data, headers=auth_headers)
...
def test_create_private_topic_no_allowed_users(client: TestClient, auth_headers):
    """Test private topic creation without allowed users succeeds (creator gets auto-access)"""
    topic_data = {
        "title": "Private Topic No Users",
        "answers": ["Yes", "No"],
        "is_public": False,
        "allowed_users": []  # Private but no allowed users - creator gets auto-access
    }
    ...
    # Current behavior: Private topics without allowed_users are allowed (creator gets auto-access)
    assert response.status_code == 200
    data = response.json()
    assert "share_code" in data
    assert data["title"] == "Private Topic No Users"


def test_get_topic_success(client: TestClient, auth_headers, sample_topic_data):
    """Test getting topic details"""
    # First create a topic
    create_response = client.post("/api/topics", json=sample_topic_data, headers=auth_headers)
...
def test_get_nonexistent_topic(client: TestClient, auth_headers):
    ...
    response = client.get("/api/topics/invalidcode", headers=auth_headers)
...
def test_vote_on_topic_success(client: TestClient, auth_headers, sample_topic_data):
    ...
    topic_response = client.get(f"/api/topics/{share_code}", headers=auth_headers)
...
def test_vote_invalid_choice(client: TestClient, auth_headers, sample_topic_data):
    ...
    vote_data = {"choices": ["Invalid Option"]}
    response = client.post(f"/api/topics/{share_code}/votes", json=vote_data, headers=auth_headers)
